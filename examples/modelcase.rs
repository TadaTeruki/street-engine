...
use city_engine::core::container::network::Network;
...
fn main() {
    ...
    let network = TransportBuilder::new(&map_provider)
        .add_origin(Site { x: 0.0, y: 0.0 }, 0.0, 0.2)
        .unwrap()
        .iterate_n_times(100)
        .build();
...
fn write_to_image(
    ...
    network.nodes().iter().for_each(|node_i| {
        // draw node
        let site = node_i.site;
        ...
        network.neighbors_iter(*node_i).unwrap().for_each(|node_j| {
            let site_a = node_i.site;
            let site_b = node_j.site;
