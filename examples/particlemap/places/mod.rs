...
fn create_place_map<
    T: PlaceNodeAttributes,
    E: PlaceNodeEstimator<T>,
    U: Debug + Clone + Copy + PartialEq + Sync,
>(
    place_particle_param: ParticleParameters,
    place_node_estimator: E,
    base_map: &ParticleMap<U>,
) -> ParticleMap<PlaceNode<T>> {
    let places = base_map
        .iter()
        .par_bridge()
        .map(|(base_particle, _)| {
...
fn draw_place_map<T: PlaceNodeAttributes>(
    particle_map: &ParticleMap<PlaceNode<T>>,
    ...
    for (_, node) in particle_map.iter() {
        ...
        cr.arc(
            x,
            y,
            particle_map.params().scale * 30.0 + 3.0,
            0.0,
