...
use crate::{
    ...
    system::{
        growth::stump::Stump,
        node::{nodetype::TransportNodeType, numeric::Stage, TransportNode},
        path::{
            factor::{metrics::TransportMetrics, PathConstructionFactors},
            TransportPath,
        },
    },
    ...
    TP: TerrainProvider,
{
    ...
    /// The path which is extended from `origin_site` by `direction_radian` (and the opposite path) will be the first stump_heap.
    /// Return `Some` if the origin node is added successfully.
    /// Return `None` if the origin node is not added successfully.
    pub fn add_origin(
        mut self,
        path_network: &mut PathNetwork<TransportNode, TransportPath, PathConstructionFactors>,
        origin_site: Site,
    ...
    fn push_new_stump(
        &mut self,
        path_network: &PathNetwork<TransportNode, TransportPath, PathConstructionFactors>,
        node_id: NodeId,
        direction: Angle,
        stage: Stage,
    ) -> Option<Stump> {
...
use super::{
    checker::path_checker::{self, PathChecker}, node::{growth_type::GrowthType, node_stump::NodeStump, transport_node::TransportNode}, params::{
        numeric::Stage, prioritization::PathPrioritizationFactors, rules::GrowthRules, StumpParams,
    }, path_network_repository::PathNetworkRepository, traits::{GrowthRulesProvider, PathPrioritizator, RandomF64Provider, TerrainProvider}
};
    ...
    PE: PathPrioritizator,
{
            ...
            .filter_map(|angle| {
                for i in 0..=rules_start.bridge_rules.check_step {
                    ...
                    {/*
                        ...
                        */
                        let path_checker = PathChecker::new(self.rules_provider, self.terrain_provider);
                        if path_checker.check_path_type(
    ...
    pub fn determine_growth_of_stump(
        ...
        let growth = stump.determine_growth(
            ...
            PathChecker::new(self.rules_provider, self.terrain_provider),
        );
