...
mod tests {
    ...
    fn test_get_intersection() {
        ...
        let line0 = Path::new_linear(Site::new(0.0, 0.0), Site::new(2.0, 2.0));
        let line1 = Path::new_linear(Site::new(1.0, 1.0), Site::new(3.0, 3.0));
        ...
        let line0 = Path::new_linear(Site::new(1.0, 1.0), Site::new(3.0, 3.0));
        let line1 = Path::new_linear(Site::new(2.0, 2.0), Site::new(4.0, 4.0));
        assert_eq!(line0.get_intersections(&line1).get(0), None);

        // Intersecting at an end point
        let line0 = Path::new_linear(Site::new(0.0, 0.0), Site::new(2.0, 0.0));
        let line1 = Path::new_linear(Site::new(2.0, 0.0), Site::new(2.0, 2.0));
        assert_eq!(
        ...
        let line0 = Path::new_linear(Site::new(0.0, 1.0), Site::new(4.0, 1.0));
        let line1 = Path::new_linear(Site::new(2.0, 0.0), Site::new(2.0, 3.0));
        assert_eq!(
            line0.get_intersections(&line1).get(0),
        ...
        let line0 = Path::new_linear(Site::new(0.0, 0.0), Site::new(1.0, 1.0));
        let line1 = Path::new_linear(Site::new(1.0, 1.0), Site::new(2.0, 2.0));
        assert_eq!(line0.get_intersections(&line1).get(0), None);

        // Lines with no intersection (completely separate)
        let line0 = Path::new_linear(Site::new(0.0, 0.0), Site::new(1.0, 1.0));
        let line1 = Path::new_linear(Site::new(2.0, 2.0), Site::new(3.0, 3.0));
        assert_eq!(line0.get_intersections(&line1).get(0), None);

        // Edge case: Zero-length line segment
        // This is expected to return None.
        let line0 = Path::new_linear(Site::new(1.0, 1.0), Site::new(1.0, 1.0));
        let line1 = Path::new_linear(Site::new(1.0, 1.0), Site::new(2.0, 2.0));
        assert_eq!(line0.get_intersections(&line1).get(0), None);

        // Intersecting at a point
        let line0 = Path::new_linear(Site::new(1.0, 3.0), Site::new(3.0, 4.0));
        let line1 = Path::new_linear(Site::new(1.0, 4.0), Site::new(2.0, 2.0));
        assert_eq!(
            line0.get_intersections(&line1).get(0),
    ...
    fn test_get_projection() {
        let line = Path::new_linear(Site::new(1.0, 1.0), Site::new(3.0, 3.0));
        let site = Site::new(1.0, 3.0);
        assert_eq!(line.get_projection(&site), Some(Site::new(2.0, 2.0)));

        let line = Path::new_linear(Site::new(1.0, 1.0), Site::new(2.0, 2.0));
        let site = Site::new(1.0, 3.0);
        assert_eq!(line.get_projection(&site), Some(Site::new(2.0, 2.0)));

        let line = Path::new_linear(Site::new(1.0, 1.0), Site::new(1.0, 2.0));
        let site = Site::new(1.0, 3.0);
