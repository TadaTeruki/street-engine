    ...
    TP: TerrainProvider,
{
    ...
    pub fn estimate_path_type(&self, node: TransportNode) -> PathType {
        /*
        if let Some(node) = self.create_checknode(node, node) {
            if self.node_constructs_bridge(&node, node.site) {
                PathType::Bridge
            } else {
                PathType::Normal
            }
    ...
    pub fn check_path_type(&self, node0: TransportNode, node1: TransportNode) -> PathType {
